...
const ComingSoon = () => {
  ...
  const loadUserData = () => {
    // NOTE: This mock data is intentionally duplicated here for this single file component
    // In a real app, this logic would live in useHome.js
    const mockEvents = [
      {
        id: 'event1',
        type: 'event',
    ...
    // Determine the single closest upcoming item
    findNextUpItem([...mockEvents, ...mockClasses]);
  };
  ...
  const findNextUpItem = (items) => {
    ...
    const futureItems = items.filter((i) => i.date > now);
    if (futureItems.length === 0) return;
    const nextItem = futureItems.sort((a, b) => a.date.getTime() - b.date.getTime())[0];
    setNextUpItem(nextItem);
  };
  
  // Helper to format date for the card title
  const formatCardTitleDate = (date) => {
  ...
  const NextUpSection = () => {
    ...
    // The main title text combines the title and the formatted time
    const titleText = `${nextUpItem.title} · ${formatCardTitleDate(nextUpItem.date)}`;
      ...
      <View style={styles.section}>
        ...
        <View style={styles.nextUpCardWrapper}>
            ...
            <View style={styles.nextUpContent}>
                <Text style={styles.nextUpTitle}>{titleText}</Text>
                
                <TouchableOpacity style={styles.nextUpButton}>
...
// --- STYLESHEET (Updated for Premium Look) ---
const CARD_BACKGROUND = '#FFFFFF';
const BACKGROUND_LIGHT = '#F9F9FA';
const ACCENT_ORANGE = '#FF6347'; // FAB/Next Up primary color
...
const styles = StyleSheet.create({
  ...
  // --- Next Up Card Styles (Matching Mockup) ---
  nextUpLabel: {
    fontSize: 10,
    fontWeight: '700',
    color: '#8A8A8A',
  ...
  nextUpGradient: {
    ...
    // MOCK: The gradient color mix from the original mockup
  ...
  nextUpTitle: {
    fontSize: 18,
    fontWeight: '900', 
    color: CARD_BACKGROUND, // White text on gradient
  ...
  nextUpButton: {
    ...
    // Subtle shadow for the button inside the card
    ...Platform.select({
      ios: {
        shadowColor: '#000',
  ...
  // --- Placeholder Styles (Kept small as this file only handles the card) ---
  card: { backgroundColor: CARD_BACKGROUND, borderRadius: 16, overflow: 'hidden', marginBottom: 16, },
  shadow: { /* defined by nextUpCardWrapper */ },
  horizontalCard: { width: 260, marginRight: 16, },
